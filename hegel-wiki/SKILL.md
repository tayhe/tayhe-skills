...
description: |
  哲学知识库维护技能。遵循 Karpathy LLM Wiki 方法论，以辩证运动为中心组织哲学知识。
...
以辩证运动为中心的哲学知识库操作指南。知识库遵循 Karpathy 的 LLM Wiki 方法论——wiki 是持久的、复利增长的产物，LLM 负责维护，人类负责策展和提问。

## 前置条件

执行任何操作前，先读取目标知识库的 `AGENTS.md` 了解其具体目录结构和约定。标准路径格式：`~/Documents/Philosophie-studien/`
...
│   │   ├── Bewegungen/             # 运动笔记（中心层）
│   │   ├── Momente/                # 环节笔记（运动的阶段）
│   │   ├── Spannungen/             # 张力笔记（开放性问题）
│   │   └── Anwendungen/            # 应用笔记（实践分析）
...
3. **创建/更新页面** — 按顺序：概念→运动→环节→张力→应用→更新现有页面。每创建一个页面，立即更新所有相关页面的 `[[]]` 引用
...
**添加哲学家**：在 `Wiki/` 下建目录 → 创建五层子目录 → 在 `Raw/Primärquellen/` 下建目录 → 更新索引
