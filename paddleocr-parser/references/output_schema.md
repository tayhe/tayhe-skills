# PaddleOCR Parser Output Schema

This document defines the output envelope returned by `paddleocr_parse.py`.

By default, `paddleocr_parse.py` saves the JSON envelope to a unique file under the system temp directory and prints the absolute saved path to `stderr`. Use `--output` when you need a custom destination, or `--stdout` when you want to skip file saving and print JSON directly.

## Output Envelope

`paddleocr_parse.py` wraps provider response in a stable structure:
...
`paddleocr_parse.py` extracts top-level `text` from `result.result.layoutParsingResults[n].markdown.text` and joins pages with `\n\n`.

## Command Examples

```bash
# Parse document from URL (result auto-saves to the system temp directory)
uv run scripts/paddleocr_parse.py --file-url "URL" --pretty

# Parse local file (result auto-saves to the system temp directory)
uv run scripts/paddleocr_parse.py --file-path "doc.pdf" --pretty

# Parse with explicit file type
uv run scripts/paddleocr_parse.py --file-url "URL" --file-type 1 --pretty

# Save result to a custom file path
uv run scripts/paddleocr_parse.py --file-url "URL" --output "./result.json" --pretty

# Print JSON to stdout without saving a file
uv run scripts/paddleocr_parse.py --file-url "URL" --stdout --pretty

# Parse with async mode for large documents
uv run scripts/paddleocr_parse.py --file-path "large.pdf" --async-mode --pretty
```