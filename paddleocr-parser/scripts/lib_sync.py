"""
PaddleOCR Document Parsing Library — Sync API

Simple document parsing API wrapper for PaddleOCR (synchronous).
"""
...
def _get_env_compat(key: str, *fallbacks: str) -> str:
    ...
    Tries primary key first, then each fallback in order.
...
def get_config() -> tuple[str, str]:
    ...
    Env vars (with fallback chain):
      API URL:  PADDLEOCR_SYNC_API_URL → PADDLEOCR_DOC_PARSING_API_URL → PADDLEOCR_API_URL
      Token:    PADDLEOCR_ACCESS_TOKEN → PADDLEOCR_ASYNC_TOKEN
    ...
    if not api_url:
        raise ValueError(
            f"PADDLEOCR_SYNC_API_URL not configured. Get your API at: {API_GUIDE_URL}"
        )
    if not token:
        raise ValueError(
    ...
    api_url = _resolve_api_url(api_url, "PADDLEOCR_SYNC_API_URL")
    api_path = urlparse(api_url).path.rstrip("/")
    if not api_path.endswith("/layout-parsing"):
        raise ValueError(
            "PADDLEOCR_SYNC_API_URL must be a full endpoint ending with "
            "/layout-parsing. "
